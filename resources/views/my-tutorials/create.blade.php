            ...
            <form action="{{ route('my-tutorials.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" id="content-field">
                    ...
                    <textarea 
                        ...
                        rows="10"
                        placeholder="Écrivez ici le contenu détaillé de votre tutoriel..."
                    >{{ old('content') }}</textarea>
                    <span class="form-text">Vous pouvez écrire le tutoriel directement ici ou joindre un fichier ci-dessous</span>
                    @error('content')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>

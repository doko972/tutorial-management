                ...
                <div class="form-group">
                    ...
                    <select id="branch_id" name="branch_id" class="form-control @error('branch_id') has-error @enderror" required>
                        ...
                        @foreach($branches as $branch)
                            <!-- Branche parente -->
                            <option value="{{ $branch->id }}" {{ old('branch_id', $myTutorial->branch_id) == $branch->id ? 'selected' : '' }} style="font-weight: 600;">
                                📂 {{ $branch->name }}
                            </option>
                            
                            <!-- Sous-branches -->
                            @foreach($branch->children as $child)
                                <option value="{{ $child->id }}" {{ old('branch_id', $myTutorial->branch_id) == $child->id ? 'selected' : '' }} style="padding-left: 20px;">
                                    &nbsp;&nbsp;&nbsp;&nbsp;└─ {{ $child->name }}
                                </option>
                            @endforeach
                        @endforeach

            ...
            <div>
                <div class="card" style="padding: 2rem;">
                    ...
                    @if($tutorial->thumbnail)
                        <img src="{{ Storage::url($tutorial->thumbnail) }}" alt="{{ $tutorial->title }}" style="width: 100%; height: auto; border-radius: 0.75rem; margin-bottom: 2rem;">
                    @endif
...
@if($tutorial->file_path || $tutorial->video_url)
    <div style="margin-top: 2rem; padding: 1.5rem; background: #f1f5f9; border-radius: 0.75rem;">
        ...
        @elseif($tutorial->file_path)
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 1rem; background: white; border-radius: 0.5rem; border: 1px solid #e2e8f0;">
                <div style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <div>
                        ...
                        <div style="font-size: 0.875rem; color: #64748b;">{{ number_format($tutorial->file_size / 1024, 2) }} KB</div>
                    </div>
                </div>
                <a href="{{ Storage::url($tutorial->file_path) }}" download class="btn btn-primary btn-sm">
...
@endif
                    ...
                    @if($tutorial->tags->count() > 0)
                        <div style="margin-top: 2rem; padding-top: 2rem; border-top: 1px solid #e2e8f0;">
                            <h3 style="font-size: 1rem; font-weight: 600; color: #0f172a; margin-bottom: 1rem;">Tags</h3>
                            <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                                @foreach($tutorial->tags as $tag)
                                    <span style="padding: 0.25rem 0.75rem; background: #f1f5f9; color: #475569; border-radius: 9999px; font-size: 0.875rem;">
                                        #{{ $tag->name }}
                                    </span>
                                @endforeach
                            </div>
            ...
            <div>
                ...
                @can('update', $tutorial)
                    <div class="card" style="margin-bottom: 1.5rem; padding: 1.5rem;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                            <a href="{{ route('my-tutorials.edit', $tutorial) }}" class="btn btn-primary btn-block">
                                <svg style="width: 1rem; height: 1rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                Modifier
                            </a>
                            <form action="{{ route('my-tutorials.destroy', $tutorial) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce tutoriel ?');">
                                @csrf
                                @method('DELETE')
                ...
                @if($relatedTutorials->count() > 0)
                    <div class="card" style="padding: 1.5rem;">
                        <h3 style="font-size: 1.125rem; font-weight: 600; color: #0f172a; margin-bottom: 1rem;">Tutoriels similaires</h3>
                        <div style="display: flex; flex-direction: column; gap: 1rem;">
                            @foreach($relatedTutorials as $related)
                                <a href="{{ route('tutorials.show', $related) }}" style="display: block; padding: 1rem; background: #f8fafc; border-radius: 0.5rem; text-decoration: none; transition: background 0.3s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='#f8fafc'">
                                    <h4 style="font-size: 0.875rem; font-weight: 600; color: #0f172a; margin-bottom: 0.5rem;">{{ $related->title }}</h4>
                                    <p style="font-size: 0.75rem; color: #64748b; margin: 0;">{{ $related->author->name }} • {{ $related->views_count }} vue(s)</p>
                                </a>
                            @endforeach
                        </div>
                    </div>

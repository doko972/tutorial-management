...
@section('content')
    <div class="dashboard-section">
        ...
        @if(session('success'))
            <div
                style="margin-bottom: 1.5rem; padding: 1rem; background: #d1fae5; color: #065f46; border-radius: 0.5rem; border-left: 4px solid #10b981;">
                {{ session('success') }}
            </div>
        @endif
        ...
        @if(session('error'))
            <div
                style="margin-bottom: 1.5rem; padding: 1rem; background: #fee2e2; color: #991b1b; border-radius: 0.5rem; border-left: 4px solid #ef4444;">
                {{ session('error') }}
            </div>
        @endif
            ...
            @foreach($branches as $branch)
                <div class="card" style="border-top: 4px solid {{ $branch->color }};">
                    <div style="display: flex; align-items: start; justify-content: space-between; margin-bottom: 1rem;">
                        ...
                        <div
                            style="width: 3rem; height: 3rem; border-radius: 0.5rem; background: {{ $branch->color }}20; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                            @if($branch->icon)
                    ...
                    <!-- Statistiques -->
                    <div
                        style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; padding: 1rem; background: #f8fafc; border-radius: 0.5rem; margin-bottom: 1rem;">
                    ...
                    <div style="font-size: 0.75rem; color: #94a3b8; margin-bottom: 1rem;">
                        ...
                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                            ...
                            <span
                                style="display: inline-block; width: 1rem; height: 1rem; background: {{ $branch->color }}; border-radius: 0.25rem; border: 1px solid #e2e8f0;"></span>
                            {{ $branch->color }}
                        </div>
                    </div>
